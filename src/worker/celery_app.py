...
app.conf.beat_schedule = {
    ...
    "self-heal-pipeline": {
        "task": "src.worker.celery_app.self_heal_pipeline_task",
...
@app.task(name="src.worker.celery_app.self_heal_pipeline_task")
def self_heal_pipeline_task() -> None:
    logger.info("Running self_heal_pipeline_task via Celery")
    cmd = [sys.executable, "-m", "src.pipeline.self_heal"]
    ...
    if completed.returncode != 0:
        ...
        logger.error(
            "self_heal_pipeline_task failed (returncode=%d)\n%s",
            completed.returncode,
            stderr_tail,
        )
    else:
        logger.info("self_heal_pipeline_task completed (returncode=0)")
