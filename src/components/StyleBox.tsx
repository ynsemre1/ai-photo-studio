...
export default function StyleBox({ uri, value, onPress, size }: Props) {
  ...
  const starAnimatedStyle = useAnimatedStyle(() => {
    return {
      transform: [{ scale: starScale.value }, { rotate: `${starRotation.value}deg` }],
    }
  })
      ...
      <TouchableOpacity
        ...
        style={[
          styles.item,
          {
            width: finalSize,
            height: finalSize,
            backgroundColor: scheme === "dark" ? colors.surface[100] : colors.primary[50],
          },
        ]}
        activeOpacity={0.9}
      >
        <View style={styles.imageWrapper}>
          ...
          {error ? (
            ...
            >
              ...
              <Text style={[styles.errorText, { color: colors.text.secondary }]}>Image failed to load</Text>
            </View>
          ) : (
            <>
              <Image
          ...
          <Animated.View style={[styles.starIconContainer, starAnimatedStyle]}>
            <RNAnimated.View style={{ transform: [{ scale: pulseAnim }] }}>
              ...
              >
                <AntDesign name={isFav ? "star" : "staro"} size={18} color={isFav ? "#FFD700" : colors.text.primary} />
              </TouchableOpacity>
            </RNAnimated.View>
          </Animated.View>
